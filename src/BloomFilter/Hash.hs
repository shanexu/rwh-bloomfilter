{-# LANGUAGE BangPatterns, ForeignFunctionInterface, FlexibleInstances, UndecidableInstances #-}
...
hashStorable salt k =
  unsafePerformIO . with k $ \ptr -> hashIO ptr (fromIntegral (sizeOf k)) salt
...
hashList salt xs =
  withArrayLen xs $ \len ptr -> hashIO ptr (fromIntegral (len * sizeOf x)) salt
  where
    x = head xs
...
hashByteString salt bs =
  Strict.useAsCStringLen bs $ \(ptr, len) -> hashIO ptr (fromIntegral len) salt
